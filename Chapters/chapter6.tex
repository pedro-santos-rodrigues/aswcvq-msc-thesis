...
An one-way analysis of variance (ANOVA) was performed to determine if there is statistically significant differences between the users success rate using the existing interface and the final prototype.

\textbf{Achived Scenarios:}

Accordingly, the first ANOVA was used to determine the differences between the achived scenarios (stats presented in table \ref{tab:statistical_summary_achieved}) in the existing interface and the final prototype.

\begin{center}
  H0 - "There is no significant difference between the achieved scenarios rate in the existing interface and in the final prototype"
\end{center}

A one-way between subjects ANOVA was conducted (table \ref{tab:anova_achieved} to compare if the mean between the samples is equal. In a 5\% confidence interval, the null hypothesis is rejected, meaning that the two means are statistically different between the two tests (p-value = 1.24E-06 < 0,05). Therefore, it can be concluded that there is a statistically significant difference in terms of the achieved scenarios between the existing interface and the final prototype tests.
...
The second ANOVA was used to determine the differences between the not achieved scenarios (stats presented in table \ref{tab:statistical_summary_not_achieved}) in the existing interface and the final prototype.

\begin{center}
  H0 - "There is no significant difference between the not achieved scenarios rate in the existing interface and in the final prototype"
\end{center}

A one-way between subjects ANOVA was conducted (table \ref{tab:anova_not_achieved} to compare if the mean between the samples is equal. In a 5\% confidence interval, the null hypothesis is rejected, meaning that the two means are statistically different between the two tests (p-value = 5.08E-05 < 0,05). Therefore, it can be concluded that there is a statistically significant difference in terms of the not achieved scenarios between the existing interface and the final prototype tests.
