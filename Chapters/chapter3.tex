...
\begin{figure}[htbp]
  \centering
  \subcaptionbox{Reisner Model\label{fig:reisner_model} (adapted from \cite{humanFactorStudiesOfDatabaseQueryLanguages})} %
    {\includegraphics[width=0.9\linewidth]{reisner-model}}%
    \\
  \subcaptionbox{Ogden Model \label{fig:ogden_model} (adapted from \cite{implicationsOfACognitiveModelOfDatabaseQuery})}%
