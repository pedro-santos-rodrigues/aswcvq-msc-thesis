...
%FINAL REPORT: Thus, if from one side the technological evolution and the digital transformation have optimized the data querying process, only a subset of people could use these powerful querying technologies.
...
%FINAL REPORT:, in comparison with those made in a textual language by expert users. 
...
%FINAL REPORT: Therefore, it is provided to users an alternative to perform their requests that are not supported by the low-code visual approaches. 
...
Under the above mentioned circumstances, the goal of this thesis is to design and evaluate a new and more powerful \gls{VQI} to provide an improved user experience that allows developers to complex data queries intuitively and efficiently, without using \gls{SQL}.
