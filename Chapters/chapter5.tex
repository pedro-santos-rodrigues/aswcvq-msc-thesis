...
The first topic defined was the type of testing scenarios that would be proposed. For this decision, it was taken into account specific aspects that should be improved in the interface usability. As mentioned, optimization of the efficiency, effectiveness, learnability, as well as user satisfaction of the entire query formulation process was the main goal, but also the improvement of the query comprehension. So, it was important to evaluate if users could understand the query purpose (i.e., what data was intended to be fetched from the database) as well as the time they required to realize that.

Considering those evaluation requirements, two types of testing scenarios were prepared: scenarios where users explore an existing query with the purpose to find the fetched information, and the other one where users would try to formulate a new query on their own. Through this approach, it was possible to analyze the usability of the interfaces on the following points of view: comprehension and formulation
